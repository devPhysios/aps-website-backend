...
const createQuestion = async (req, res) => {
    ...
    const question = await MCQQuestion.create({ ...req.body });
...
const editQuestion = async (req, res) => {
    ...
    try {
        const question = await MCQQuestion.findOneAndUpdate(
...
const deleteQuestion = async (req, res) => {
    const { questionId } = req.params;
    try {
        const question = await MCQQuestion.findOneAndDelete({ _id: questionId, authorId: req.student.studentId });
        if (!question) {
            throw new NotFoundError('Question not found');
        }
