...
const createQuestion = async (req, res) => {
    ...
    const question = await FillInTheGapQuestion.create({ ...req.body });
...
const getQuestions = async (req, res) => {
    ...
    const questions = await FillInTheGapQuestion.find(filter);
...
const editQuestion = async (req, res) => {
    ...
    try {
        const question = await FillInTheGapQuestion.findOneAndUpdate(
...
const deleteQuestion = async (req, res) => {
    const { questionId } = req.params;
    try {
        const question = await FillInTheGapQuestion.findOneAndDelete({ _id: questionId, authorId: req.student.studentId });
        if (!question) {
            throw new NotFoundError('Question not found');
        }
