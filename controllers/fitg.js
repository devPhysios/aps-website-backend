...
const createQuestion = async (req, res) => {
  try {
    ...
    const fillInTheGapQuestion = new FillInTheGapQuestion({
      question,
      imgURL,
      answer,
      courseCode,
...
const getQuestions = async (req, res) => {
  try {
    ...
    const questions = await FillInTheGapQuestion.find({ courseCode })
      .sort({ year: -1 })
...
const editQuestion = async (req, res) => {
    try {
        ...
        const question = await FillInTheGapQuestion.findById(id);
        if (!question) {
